"""Create controlled Mini-DIO probe variant O.

Variant O is a deterministic follow-up after N. It gives the persistent latent
dio_0325 trace another passive world contact and tests whether the trace fades,
stays latent, or recouples toward the reduced dio_10qz/dio_114i pair.
...
def mutate_rows(rows: list[dict], probe: int) -> list[dict]:
    """Return a deterministic seventh-contact continuation of variant N."""

    if not rows:
        return []
    timestamp_shift = 4_700_000_000_000 + (probe * 41_000_000)
    probe_phase = ((probe % 17) - 8) / 8.0
    mutated: list[dict] = []
    ...
    for index, row in enumerate(rows):
        ...
        local_phase = ((index % 41) - 20) / 20.0
        slow_wave = ((index // 8) % 15 - 7) / 7.0
        pulse = ((index % 18) - 8.5) / 8.5
        return_wave = ((index // 7) % 7 - 3) / 3.0
        ...
        body_scale = max(0.55, 1.0 + (return_wave * 0.014) - (pulse * 0.013))
        upper_scale = max(0.55, 1.0 + (abs(slow_wave) * 0.014) + (local_phase * 0.009))
        lower_scale = max(0.55, 1.0 + (abs(return_wave) * 0.016) - (local_phase * 0.010))
        volume_scale = max(0.78, min(1.22, 1.0 + (abs(pulse) * 0.014) + (abs(return_wave) * 0.014)))
...
def create_variant_o(data_dir: Path) -> list[Path]:
    written: list[Path] = []
    for probe, label in PROBES.items():
        source = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteN_folgekontakt5_5m_SOLUSDT.csv"
        target = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteO_folgekontakt6_5m_SOLUSDT.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create controlled Mini-DIO variant O CSVs.")
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    for path in create_variant_o(Path(args.data_dir)):
        print(path)
