"""Create controlled Mini-DIO probe variant I.

Variant I is a near-world follow-up after H. It exists to test whether weak
single-probe carried traces can absorb more world contact. It mutates only
...
def mutate_rows(rows: list[dict], probe: int) -> list[dict]:
    """Return a deterministic near-world contact continuation of variant H."""

    if not rows:
        return []
    timestamp_shift = 2_100_000_000_000 + (probe * 16_000_000)
    probe_breath = ((probe % 5) - 2) * 0.0015
    mutated: list[dict] = []
    ...
    for index, row in enumerate(rows):
        ...
        phase = ((index % 17) - 8) / 8.0
        slow_wave = ((index // 7) % 5 - 2) / 2.0
        local_breath = ((index % 6) - 2.5) / 2.5

        center_shift = (phase * 0.006) + (slow_wave * 0.0045) + probe_breath
        body_scale = max(0.60, 1.0 + (local_breath * 0.045) - (phase * 0.020))
        upper_scale = max(0.60, 1.0 + (phase * 0.035) + (slow_wave * 0.018))
        lower_scale = max(0.60, 1.0 - (phase * 0.030) + (abs(local_breath) * 0.020))
        volume_scale = max(0.82, min(1.18, 1.0 + (abs(slow_wave) * 0.030) + (abs(phase) * 0.020)))
...
def create_variant_i(data_dir: Path) -> list[Path]:
    written: list[Path] = []
    for probe, label in PROBES.items():
        source = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteH_strukturkontrast_5m_SOLUSDT.csv"
        target = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteI_weltkontakt_5m_SOLUSDT.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create controlled Mini-DIO variant I CSVs.")
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    for path in create_variant_i(Path(args.data_dir)):
        print(path)
