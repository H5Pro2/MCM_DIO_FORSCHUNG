"""Create controlled Mini-DIO probe variant P.

Variant P is a deterministic follow-up after O. It gives the latent two-family
islands another passive world contact and tests whether they remain stable,
grow, or drift apart.
...
def mutate_rows(rows: list[dict], probe: int) -> list[dict]:
    """Return a deterministic eighth-contact continuation of variant O."""

    if not rows:
        return []
    timestamp_shift = 5_300_000_000_000 + (probe * 43_000_000)
    probe_phase = ((probe % 19) - 9) / 9.0
    mutated: list[dict] = []
    ...
    for index, row in enumerate(rows):
        ...
        local_phase = ((index % 43) - 21) / 21.0
        slow_wave = ((index // 7) % 17 - 8) / 8.0
        pulse = ((index % 20) - 9.5) / 9.5
        bridge_wave = ((index // 9) % 7 - 3) / 3.0
        ...
        body_scale = max(0.55, 1.0 + (bridge_wave * 0.011) + (pulse * 0.010))
        upper_scale = max(0.55, 1.0 + (abs(local_phase) * 0.012) - (slow_wave * 0.008))
        lower_scale = max(0.55, 1.0 + (abs(slow_wave) * 0.012) + (bridge_wave * 0.008))
        volume_scale = max(0.78, min(1.22, 1.0 + (abs(bridge_wave) * 0.012) + (abs(pulse) * 0.010)))
...
def create_variant_p(data_dir: Path) -> list[Path]:
    written: list[Path] = []
    for probe, label in PROBES.items():
        source = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteO_folgekontakt6_5m_SOLUSDT.csv"
        target = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteP_folgekontakt7_5m_SOLUSDT.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create controlled Mini-DIO variant P CSVs.")
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    for path in create_variant_p(Path(args.data_dir)):
        print(path)
