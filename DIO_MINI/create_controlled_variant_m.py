"""Create controlled Mini-DIO probe variant M.

Variant M is a deterministic follow-up after L. It keeps the controlled probe
structure and adds another world-contact movement to test whether the open
multi-center bridge around dio_10qz/dio_1ytc remains carried, drifts, or forms
another peripheral island.
...
def mutate_rows(rows: list[dict], probe: int) -> list[dict]:
    """Return a deterministic fifth-contact continuation of variant L."""

    if not rows:
        return []
    timestamp_shift = 3_600_000_000_000 + (probe * 31_000_000)
    probe_phase = ((probe % 13) - 6) / 6.0
    mutated: list[dict] = []
    ...
    for index, row in enumerate(rows):
        ...
        local_phase = ((index % 31) - 15) / 15.0
        slow_wave = ((index // 9) % 11 - 5) / 5.0
        pulse = ((index % 14) - 6.5) / 6.5

        center_shift = (local_phase * 0.0027) + (slow_wave * 0.0041) + (probe_phase * 0.0013)
        body_scale = max(0.55, 1.0 + (pulse * 0.024) + (slow_wave * 0.012))
        upper_scale = max(0.55, 1.0 + (local_phase * 0.019) + (abs(pulse) * 0.016))
        lower_scale = max(0.55, 1.0 - (local_phase * 0.020) + (abs(slow_wave) * 0.018))
        volume_scale = max(0.78, min(1.22, 1.0 + (abs(pulse) * 0.022) + (abs(local_phase) * 0.015)))
...
def create_variant_m(data_dir: Path) -> list[Path]:
    written: list[Path] = []
    for probe, label in PROBES.items():
        source = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteL_folgekontakt3_5m_SOLUSDT.csv"
        target = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteM_folgekontakt4_5m_SOLUSDT.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create controlled Mini-DIO variant M CSVs.")
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    for path in create_variant_m(Path(args.data_dir)):
        print(path)
