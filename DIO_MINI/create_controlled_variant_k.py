"""Create controlled Mini-DIO probe variant K.

Variant K is a deterministic follow-up after J. It keeps the controlled probe
structure and adds another world-contact movement. The goal is to test whether
the open triad around dio_10qz/dio_114i/dio_1ytc remains bridge-held, closes,
drifts, or dissolves under an additional passive follow-up world.
...
def mutate_rows(rows: list[dict], probe: int) -> list[dict]:
    """Return a deterministic third-contact continuation of variant J."""

    if not rows:
        return []
    timestamp_shift = 2_700_000_000_000 + (probe * 23_000_000)
    probe_phase = ((probe % 9) - 4) / 4.0
    mutated: list[dict] = []
    ...
    for index, row in enumerate(rows):
        ...
        local_phase = ((index % 23) - 11) / 11.0
        slow_wave = ((index // 7) % 7 - 3) / 3.0
        pulse = ((index % 10) - 4.5) / 4.5

        center_shift = (local_phase * 0.0038) - (slow_wave * 0.0049) + (probe_phase * 0.0021)
        body_scale = max(0.56, 1.0 - (pulse * 0.030) + (slow_wave * 0.018))
        upper_scale = max(0.56, 1.0 - (local_phase * 0.025) + (abs(slow_wave) * 0.018))
        lower_scale = max(0.56, 1.0 + (local_phase * 0.024) + (abs(pulse) * 0.012))
        volume_scale = max(0.78, min(1.22, 1.0 + (abs(slow_wave) * 0.026) + (abs(pulse) * 0.018)))
...
def create_variant_k(data_dir: Path) -> list[Path]:
    written: list[Path] = []
    for probe, label in PROBES.items():
        source = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteJ_folgekontakt_5m_SOLUSDT.csv"
        target = data_dir / f"kontrolliert_sensor_relation_probe{probe}_{label}_6episoden_varianteK_folgekontakt2_5m_SOLUSDT.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create controlled Mini-DIO variant K CSVs.")
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    for path in create_variant_k(Path(args.data_dir)):
        print(path)
