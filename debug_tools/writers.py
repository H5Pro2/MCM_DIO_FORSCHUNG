"""Debug path, buffer, and writer utilities.

`debug/` remains the runtime output directory. This module contains the code
that decides where and how debug data is written.
"""
...
_DEBUG_FILE_GROUPS = {
    ...
    "mcm_area_perception_protocol.csv": "perception",
    "mcm_market_melody_protocol.csv": "perception",
    "mcm_sensory_balance_protocol.json": "perception",
    "market_melody.wav": "perception",
    "market_melody_meta.txt": "perception",
...
def _buffered_debug_enabled(path=None, mode="a", write_once=False):
    ...
    if "/debug/" not in normalized and not normalized.startswith("debug/"):
        return False
    ...
    try:
        ...
        if profile_path not in _FILE_PROFILE_HEADER_DONE:
            ...
            with open(profile_path, "w", encoding="utf-8") as handle:
                handle.write("timestamp;path;operation;elapsed_ms;bytes_written;extra\n")
