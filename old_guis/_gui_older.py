...
VISUAL_SNAPSHOT_PATH = str(getattr(Config, "MCM_VISUAL_SNAPSHOT_PATH", "debug/bot_visual_snapshot.json") or "debug/bot_visual_snapshot.json")
INNER_SNAPSHOT_PATH = str(getattr(Config, "MCM_INNER_SNAPSHOT_PATH", "debug/bot_inner_snapshot.json") or "debug/bot_inner_snapshot.json")
CMAP = 'plasma' 
heatmap_alpha = 0.18
...
class TradeStatsGUI:
    ...
    def _read_memory_state(self):
        ...
        inner_path = str(INNER_SNAPSHOT_PATH or "debug/bot_inner_snapshot.json")
    ...
    def _update_loop(self):
        ...
        inner_state = dict(self._read_memory_state() or {})
        ...
        field_density = float(inner_state.get("field_density", 0.0) or 0.0)
        field_stability = float(inner_state.get("field_stability", 0.0) or 0.0)
        regulatory_load = float(inner_state.get("regulatory_load", 0.0) or 0.0)
        action_capacity = float(inner_state.get("action_capacity", 0.0) or 0.0)
        recovery_need = float(inner_state.get("recovery_need", 0.0) or 0.0)
        survival_pressure = float(inner_state.get("survival_pressure", 0.0) or 0.0)
